...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h1 { font-size: 1.8rem; font-weight: 700; color: #7c3aed; margin-bottom: 0.3rem; letter-spacing: -0.02em; }
  .subtitle { color: #64748b; font-size: 0.9rem; margin-bottom: 2rem; }
  .card {
    background: #1e2130; border: 1px solid #2d3148; border-radius: 12px;
    padding: 1.5rem; width: 100%; max-width: 900px; margin-bottom: 1.2rem;
  }
  .card-title { font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.08em; color: #7c3aed; margin-bottom: 1rem; font-weight: 600; }
  .uploads { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; }
  @media (max-width: 600px) { .uploads { grid-template-columns: 1fr; } }
  .drop-zone {
    border: 2px dashed #3d4163; border-radius: 10px; padding: 1.5rem;
    text-align: center; cursor: pointer; transition: border-color 0.2s, background 0.2s;
    position: relative; min-height: 180px; display: flex; flex-direction: column;
    align-items: center; justify-content: center;
  }
  .drop-zone:hover, .drop-zone.dragover { border-color: #7c3aed; background: #1a1d30; }
  ...
  .drop-zone img.preview { max-width: 100%; max-height: 150px; border-radius: 6px; object-fit: contain; margin-top: 0.5rem; }
  .drop-zone .filename { font-size: 0.75rem; color: #7c3aed; margin-top: 0.4rem; word-break: break-all; }
  ...
  .btn-download { background: #059669; color: white; border: none; border-radius: 8px; padding: 0.6rem 1.5rem; font-size: 0.9rem; font-weight: 600; cursor: pointer; transition: background 0.2s; display: inline-block; text-decoration: none; }
  .btn-download:hover { background: #047857; }
  details > summary { list-style: none; }
  details > summary::-webkit-details-marker { display: none; }
  .settings-toggle { background: none; border: none; color: #7c3aed; cursor: pointer; font-size: 0.85rem; display: flex; align-items: center; gap: 0.4rem; padding: 0; margin-bottom: 0.8rem; }
  .settings-toggle:hover { color: #a78bfa; }
</style>
</head>
<body>
<h1>SAM3 Background Replace</h1>
<p class="subtitle">Segment Anything Model 3 — person cutout &amp; background swap</p>
...
<div class="card">
  ...
  <div class="uploads">
    <div>
      ...
      <div class="drop-zone" id="zone-portrait" ondragover="onDrag(event,'zone-portrait')" ondragleave="offDrag('zone-portrait')" ondrop="onDrop(event,'portrait')">
    ...
    <div>
      ...
      <div class="drop-zone" id="zone-background" ondragover="onDrag(event,'zone-background')" ondragleave="offDrag('zone-background')" ondrop="onDrop(event,'background')">
  ...
  <details id="settings-details">
    ...
    <div class="settings-grid" style="margin-top:0.5rem;">
      <div>
        ...
        <input type="number" id="inp-fill" value="92" min="10" max="100" step="1"/>
      </div>
      <div>
        <label class="field-label" for="inp-foot">Foot Anchor (% from top)</label>
        <input type="number" id="inp-foot" value="92" min="50" max="98" step="1"/>
      </div>
      <div>
...
<script>
  ...
  function onDrop(e, key)    { e.preventDefault(); offDrag('zone-'+key); const f=e.dataTransfer.files[0]; if(f) setFile(key,f); }
  function onFile(e, key)    { const f=e.target.files[0]; if(f) setFile(key,f); }
  ...
  function setLoading(loading) {
    ...
    document.getElementById('btn-text').textContent = loading ? 'Running SAM3…' : 'Replace Background';
  ...
  async function processImages() {
    ...
    setLoading(true);
    setStatus('Segmenting person with SAM3…', 'info');
...
async def process(
    ...
    portrait_path   = TMP_DIR / f"{uid}_portrait{Path(portrait.filename or 'img.jpg').suffix or '.jpg'}"
    background_path = TMP_DIR / f"{uid}_background{Path(background.filename or 'img.jpg').suffix or '.jpg'}"
